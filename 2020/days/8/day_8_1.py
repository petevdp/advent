...
with open('days/8/input') as f:
    for line in f:
        ...
        insts.append(Inst(op, int(num_str)))


index_to_change = find_inst_to_change()
...
def find_index_to_change():
    ...
    for index in range(len(insts)):
        if not will_terminate_from_instruction(index)[0] == None:
    ...
    for index in [i, inst for inst in base_visited if starting_insts[i].op == 'nop']:
